...
test('board should place stones on intersections', async ({ browser, request }) => {
  ...
  try {
    ...
    await pageB.goto(`http://127.0.0.1:5173/?roomId=${roomId}`);
    await expect(pageB.locator('.modal-overlay')).toBeVisible();
    await pageB.getByPlaceholder(/我的昵称|Your name/i).fill(`human-b-${Date.now()}`);
    await pageB.getByRole('button', { name: /加入房间|Join Room/i }).last().click();
    await expect(pageB.locator('.modal-overlay')).toHaveCount(0);

    await expect(pageA.locator('.grid')).toBeVisible();
    await expect(pageA.locator('.grid .cell')).toHaveCount(225);

    const target = pageA.getByLabel('cell-7-7');
    await target.click();
    await expect(target.locator('.stone.black')).toBeVisible();
  } finally {
    await ctxA.close();
    await ctxB.close();
  }
...
realAgentPromptTest('real agent can join a human-created room by room-id prompt', async ({ request }) => {
  ...
  const skillBaseUrl = 'http://127.0.0.1:5173';
